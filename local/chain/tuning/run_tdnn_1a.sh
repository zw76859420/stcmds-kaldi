...
dir=${dir}${affix:+_$affix}_sp_chain
train_set=train
...
if [ $stage -le 10 ]; then
  ...
  fixed-affine-layer name=lda input=Append(-1,0,1,2) affine-transform-file=$dir/configs/lda.mat
...
if [ $stage -le 13 ]; then
  for test_set in test; do
    steps/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 \
      --nj 8 --cmd "$decode_cmd" \
      $graph_dir data/test $dir/decode_${test_set} || exit 1;
  done
fi
